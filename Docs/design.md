...
The CPLD implements a user/kernel mode bit. This is flipped to kernel mode
when the 6809's `BS` line goes high. In user mode, the whole 64K of address
...
I could keep 8K pages and this would let the MMU to address 1M of RAM.
...
`$0000` to `$7FFF` and, in user mode, the RAM from `$8000` to `$FDFF` gets mapped
through the MMU.
...
I/O devices. Have a look at the `mmu_decode.v` file which holds the CPLD code.
